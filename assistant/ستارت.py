...
# تم الاحتفاظ بجميع الزخارف (1 إلى 37) والوظائف كاملة
...
<tg-emoji emoji-id="{PREMIUM_EMOJI_ID}">👇</tg-emoji> <b>لـ البـدء إستخـدم الازرار بالاسفـل</b>'''
        ...
        try:
            ...
            LOGS.error(f"خطأ في إرسال البداية: {e}")
            fallback_buttons = []
            for row in buttons:
                btn_row = []
                for btn in row:
            ...
            if int(chat.id) in tt:
                ...
                if customtasmsg:
                    tas_msg = customtasmsg.format(
                        ...
                        username=username, userid=chat.id,
                        my_first=user.first_name, my_last=user.last_name,
                        my_zname=my_fullname, my_username=f"@{user.username}" if user.username else my_mention,
...
**⌔ تحلى بالصبـر وانتظـر الـرد 📨.**"""
            ...
            # وضع الزخرفة (يتم إرسال جميع الزخارف من WA1 إلى WA37)
            if chat.id in dd:
                text = event.text
                iitems = ['࿐', '𖣳', '𓃠', '𖡟', '𖠜', '‌♡⁩', '‌༗', '‌𖢖', '❥', '‌ঌ', '𝆹𝅥𝅮', '𖠜', '𖠲', '𖤍', '𖠛', ' 𝅘𝅥𝅮', '‌༒', '‌ㇱ', '߷', 'メ', '〠', '𓃬', '𖠄']
                # توليد 40 اختيار عشوائي (غير مستخدمة في الزخرفة النهائية لكن تم الاحتفاظ بها)
    ...
    async def callback_handler(event):
        ...
        if data == "decor_main_menu":
            if chat_id not in dd:
                dd.append(chat_id)
                await event.answer("✅ تم تفعيل وضع الزخرفة. أرسل النص المراد زخرفته (يدعم الإنجليزية فقط).", alert=True)
                await client.send_message(chat_id, "**✨ أرسل النص الإنجليزي الذي تريد زخرفته الآن:**")
            else:
                dd.remove(chat_id)
    ...
    LOGS.info("✅ تم تحميل موديول start.py مع 37 زخرفة وبدون رسالة الترحيب القديمة.")
